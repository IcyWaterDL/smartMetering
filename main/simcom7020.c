...
void uart_simcom(void *arg) {
	...
	while(1) {
		int len = uart_read_bytes(simcom_7020.uart_num, data, BUF_SIZE - 1, 100/portTICK_PERIOD_MS);
		if(len) {
			data[len] = '\0';
...
static void send_ATComand(char *ATcommand) {
	...
	uart_write_bytes(simcom_7020.uart_num, (char *)ATcommand, strlen((char *) ATcommand));
	uart_write_bytes(simcom_7020.uart_num, "\r\n", strlen("\r\n"));
...
static AT_flag _readFeedback(uint32_t timeout, char *expect) {
	...
	while(esp_timer_get_time()/1000 < (timeout + timeCurrent)) {
		...
		if(simcom_7020.AT_buff_avai) {
			if(strstr((char *)simcom_7020.AT_buff, "ERROR")) return AT_ERROR;
